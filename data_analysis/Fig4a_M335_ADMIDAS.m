...
% populate the legend text and colors you want to use for the bars
...
for state=1:length(states)
    for conditions = 1:length(subfolder)
            ...
            if isfolder(yall)
                ...
                if isfile(fullFilename) 
                    ...
                    if filesize > 40000
                        ...
                        replicadata(:,replica)=dataall(1:time,DataColumn);
                    else
                        replicadata(:,replica)=NaN(time,1);   
                    end
                else
                    replicadata(:,replica)=NaN(time,1);
                    continue
        ...
        datareplica(:,conditions,state) = mean(replicadata((time-40):time,:),'omitnan');
        replicastart(state)=dataall(1,DataColumn);
    end
end
% now that all the data is collected find the min and max for each state
% (page) and condition (column) and create the bin ranges
...
% split the replicas into bins established by the bin criteria above
...
plot(X,replicastart,'k_','MarkerSize',15,'linewidth',1)
...
yAX = get(gca,'YAxis');
set(yAX,'FontSize', 10)
ylabel({'ADMIDAS_{ion}';['to ',char(atom),' (',Ang,')']},'interpreter','tex','fontsize', 10)
xlabel('End State','fontsize', 12);
set(gcf, 'Units', 'Inches', 'Position', [1, 1, 2.965, 1.5])
yticks([0 5 10 15 20])
set(gcf,'PaperPositionMode','auto')
leg = legend(legtext,'location','northwest','fontsize', 8,'NumColumns',2);
legend('boxoff')
leg.ItemTokenSize = [8,10];
hold off
% clean everything up.
clearvars -except root1 dataindexing
walkername = fullfile(strcat(root1,'Fig4a_M335_ADMIDAS'));
